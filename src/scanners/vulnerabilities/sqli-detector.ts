...
export const createSQLiDetector = (options: SQLiDetectorOptions) => {
  ...
  const injectPayload = (url: string, param: string, payload: string, location: string): string => {
    ...
    switch (location) {
      ...
      case 'path':
        ...
        return url.replace(new RegExp(`\{${param}\}`), encodeURIComponent(payload))
      default:
  ...
  const detectSQLi = async (context: VulnerabilityTestContext) => {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      const testUrl = injectPayload(context.url, context.parameter, payload, context.parameterLocation)
      
      if (context.parameterLocation === 'query' || context.parameterLocation === 'path') {
        ...
        const contentType = context.method === 'POST' || context.method === 'PUT' 
      ...
      const analysis = await llmTester.analyzeResponse(
        context,
        {
          payload,
          technique,
      ...
      attempts.push({
        payload,
        result: analysis.isVulnerable ? 'sql_error_exposed' : 
                analysis.filterDetected ? 'blocked' : 
                testResponse.status >= 500 ? 'server_error' : 'no_change',
        response: testResponse
      })
      ...
      if (testResponse.status >= 500 && analysis.confidence > 0.6) {
        debug.sqli('Server error detected (%d), possible SQL injection', testResponse.status)
      }
      
      // If filter detected but LLM suggests bypass, continue
      if (analysis.filterDetected && analysis.suggestedNextPayload && attempt < maxAttempts - 1) {
