...
`
  ...
  const generate = (result: AnalysisResult): string => {
    ...
    if (result.vulnerabilities.length === 0) {
      ...
      output.push('')
      output.push('The scan completed successfully without finding any security issues.')
    } else {
      // Summary by severity
      ...
      for (const severity of severityOrder) {
        if (severityCounts[severity]) {
          output.push(`| ${severity.toUpperCase()} | ${severityCounts[severity]} |`)
        }
      }
      ...
      // Group by severity
      const grouped = result.vulnerabilities.reduce((acc, vuln) => {
