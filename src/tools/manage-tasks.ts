...
export const createManageTasksTool = (llm: LLMProvider): VulnAgentTool => {
  return {
    name: 'manageTasks',
    tool: tool({
      description: 'Manage scan tasks - add, update, prioritize, and retrieve tasks',
      parameters: z.object({
        sessionId: z.string().describe('Current scan session ID'),
        action: z.enum(['add', 'update', 'get', 'prioritize', 'complete'])
          .describe('Action to perform on tasks'),
        task: z.object({
          id: z.string().optional(),
          type: z.enum(['test_endpoint', 'analyze_response', 'extract_links', 'test_payload']),
          target: z.string(),
          priority: z.number().optional(),
          status: z.enum(['pending', 'in_progress', 'completed', 'failed']).optional(),
          metadata: z.record(z.unknown()).optional(),
        }).optional().describe('Task details for add/update actions'),
        filter: z.object({
          status: z.enum(['pending', 'in_progress', 'completed', 'failed']).optional(),
          type: z.string().optional(),
        }).optional().describe('Filter for get action'),
        context: z.object({
          currentFindings: z.array(z.string()).optional(),
          completedTasks: z.number().optional(),
          remainingSteps: z.number().optional(),
        }).optional().describe('Context for prioritization'),
      }),
          ...
          switch (params.action) {
            case 'add': {
              ...
              const newTask: ScanTask = {
                id: params.task.id || `task-${Date.now()}-${Math.random().toString(36).substring(7)}`,
            ...
            case 'update': {
              ...
              const taskIndex = sessionTasks.findIndex(t => t.id === params.task!.id)
              if (taskIndex === -1) {
            ...
            case 'get': {
              ...
              if (params.filter) {
                if (params.filter.status) {
                  filteredTasks = filteredTasks.filter(t => t.status === params.filter!.status)
                }
                if (params.filter.type) {
                  filteredTasks = filteredTasks.filter(t => t.type === params.filter!.type)
                }
              }
              ...
              return {
                ...
                stats: {
                  ...
                  inProgress: sessionTasks.filter(t => t.status === 'in_progress').length,
...
Current tasks:
${sessionTasks.filter(t => t.status === 'pending').map((t, i) => 
...
4. Efficiency of execution`
              ...
              // Update priorities
              result.object.prioritizedTasks.forEach(({ taskIndex, newPriority }) => {
              ...
              return {
                success: true,
                prioritizedTasks: sessionTasks.filter(t => t.status === 'pending'),
            ...
            case 'complete': {
              ...
              return {
                success: true,
                task: sessionTasks[taskIndex],
                remainingTasks: sessionTasks.filter(t => t.status === 'pending').length,
              }
            }
